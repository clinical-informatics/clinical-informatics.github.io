...
- Chapter 12 of *R for Data Science* (first edition) by Hadley Wickham and Garrett Grolemund, available free online. The chapter applies the tidy data framework to real datasets with worked-out cleaning examples in R's tidyverse. The R code is incidental; the value is seeing the framework applied to a half-dozen real-world messes in different domains, which is how a vocabulary for spotting the same patterns in clinical data gets built. [r4ds.had.co.nz/tidy-data.html](https://r4ds.had.co.nz/tidy-data.html)
